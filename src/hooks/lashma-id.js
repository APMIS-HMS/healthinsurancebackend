...
module.exports = function (options = {}) { // eslint-disable-line no-unused-vars
  return function lashmaId(hook) {
    ...
    return hook.app.service("beneficiaries").find({ query: { "platformOwnerId._id": hook.data.platformOwnerId._id } }).then(payload => {
      let bCounter = payload.data.length + 1;
      let year = new Date().getFullYear().toString().split('');
      let month = new Date().getMonth() + 1;
      let formatedM = ("0" + month).slice(-2);
      let formatedCounter = ("000" + bCounter).slice(-4);
